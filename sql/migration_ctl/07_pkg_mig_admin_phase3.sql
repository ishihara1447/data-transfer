-- PKG_MIG_ADMIN v4.0（フェーズ3 API 追加版）
-- 実行ユーザー: migration_ctl / 実行対象: oracle-tgt (localhost:1522/XEPDB1)
-- 前提: 06_mig_checkpoint.sql を先に適用しておくこと
-- 設計: docs/phase3-design.md §5
-- 注意: このファイルはパッケージ全体を再定義する。05 の18本に加え6本を追加した24本を含む。
...
CREATE OR REPLACE PACKAGE PKG_MIG_ADMIN AS
    ...
    -- -----------------------------------------------------------------------
    -- v4.0 追加 API（フェーズ3：Archived Redo 収集）
    -- 設計: docs/phase3-design.md §5
    -- -----------------------------------------------------------------------
...
CREATE OR REPLACE PACKAGE BODY PKG_MIG_ADMIN AS
    ...
    PROCEDURE REGISTER_ARCHIVE_LOG (
        ...
        v_cnt NUMBER;
    BEGIN
        -- 論理一意キーの重複は明示的に弾く（再コピーは REGISTER_ARCHIVE_LOG_COPY で行う）
        SELECT COUNT(*) INTO v_cnt
        FROM   ARCHIVE_LOG
        WHERE  MIG_RUN_ID          = p_run_id
    ...
    BEGIN
        -- Oracle 12c 互換: MERGE INTO で UPSERT を実現する
    ...
    BEGIN
        ...
        -- 条件d: ARCHIVE_COLLECTOR の全チェックポイントが TARGET_END_SCN に到達
        --        （RAC 環境では Thread 数ぶんの行を全行検査する）
        ...
        -- 条件e: 欠落した Archived Redo が存在しないこと
