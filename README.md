...
- 🔌 **オフライン前提**でも動く設計：変更を**ファイルにして運ぶ**（DBリンクや GoldenGate に依存しない）
- 📊 **進捗をひと目で確認できる監視ダッシュボード**（件数の一致・遅れ・保持リスク・適用した変更の中身まで）。
  **Webサーバ不要**の単一HTMLファイルなので、ダブルクリックで開けます
- 🗂 **移行そのものを管理する台帳**（実行ID・フェーズ進捗・SCN・Data Pumpジョブ・アーカイブログ）を DB 内に持つ（→ [12章](#12-移行管理スキーマサンプル一式)）
...
**外部参照のない単一 HTML ファイル**なので、Webサーバは不要です。生成された
`out/migration_dashboard.html` をダブルクリックすればブラウザで開き、メール添付でそのまま共有できます。
...
```
   ...
   │                                                            │         │   MIGRATION_CTL（制御：移行実行・進捗・SCN・ジョブの管理）           │
   └────────────────────────────────────────────────────────────┘        └──────────────────────────────────────────────────────────────────────┘
```
...
| 移行先DB（oracle-tgt） | **MIGRATION_CTL** | － | （制御用）**移行管理スキーマ**。実行ID・フェーズ進捗・SCN・Data Pumpジョブ・アーカイブログを一元管理（→ [12章](#12-移行管理スキーマサンプル一式)） |
...
- **なぜ移行元で解析しているのか**：変更履歴を読むには「テーブルの定義情報（辞書）」が要ります。
  当初、移行先で読ませたところ辞書が無く「変更0件」になったため、**定義がそろっている移行元で解析し、
  復元した SQL だけを運ぶ**方式にしました。
  > ⚠️ **この判断は後に見直されました。** 2026-07-26 の PoC で、辞書を変更履歴そのものに埋め込む
  > （`STORE_IN_REDO_LOGS` を **CDB$ROOT で**実行する）ことで、**移行先でも解析できる**ことを実証しています。
  > 社内の新設計は移行先で解析する方式であり、**本環境の実装とは解析場所が逆**です。
  > 経緯と実証結果は [`docs/gap-analysis-5phase-schema.md`](docs/gap-analysis-5phase-schema.md) 1章を参照。
...
| 3 | `DBMS_LOGMNR_D.BUILD` が PDB スキーマを含まない | **flat-file 辞書**は PDB オブジェクトを含まない | 当時は**解析を移行元で行い結果 SQL を搬送**する設計に変更。→ **後に #9 で解決**：redoログ埋め込み辞書（`STORE_IN_REDO_LOGS`）なら移行先でも解析できる |
