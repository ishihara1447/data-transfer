...
現状は手動調査キューに溜まるところまでで止まっており、次のいずれかの方式で
自動化することを検討しています。

| 方式 | 内容 | 評価 |
|------|------|------|
| A. 変更PK再取得 | LogMinerから変更されたPKだけ抽出→移行元から該当行を再取得→移行先へMERGE | 都度反映できるが、オフライン制約下ではPK取得のたびにファイル搬送が必要で複雑 |
| B. 最終再同期（現時点で有力） | 移行期間中はPKだけ記録しておき、最終切替時にLOB対象テーブルだけ再抽出・再ロード | 実装がシンプル。ただし切替直前の停止時間が少し増える |

> 詳細設計は [`docs/delta-extract-design.md`](docs/delta-extract-design.md) の9〜10章を参照してください。
