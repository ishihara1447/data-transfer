...
# ===========================================================================
# T02: PHASE_STATUS の INSERT と制約確認（COMPLETED。DONEは拒否）
# ===========================================================================
...
# LOW#10: STATUS='DONE' は CHK_PHASE_STATUS_STS で拒否される（ORA-02290）
DONE_ERR=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
...
if echo "${DONE_ERR}" | grep -q "ORA-02290"; then
    ...
    echo "  [NG] CHECK制約(PHASE_STATUS): STATUS='DONE' で ORA-02290 が発生しなかった"
    echo "       出力: ${DONE_ERR}"
    PASS=0
fi
...
# UNIQUE制約: 同一 MIG_RUN_ID+RESETLOGS_ID+THREAD_NO+SEQUENCE_NO → ORA-00001
UQ_ARC_ERR=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
INSERT INTO ARCHIVE_LOG (
...
INSERT INTO MIG_STATUS_HISTORY (
    MIG_RUN_ID, TABLE_NAME, RECORD_ID, OLD_STATUS, NEW_STATUS, CHANGED_BY, NOTE
...
# MED#2対応: 全コピーが VERIFIED になった時点でのみ ARCHIVE_LOG も VERIFIED になる
# ===========================================================================
...
# 1件目（MIGRATION_FILE_SERVER）を VERIFIED へ
mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON SERVEROUTPUT ON
BEGIN
...
WHERE ARCHIVE_LOG_COPY_ID = ${ALC_ID1:-0};" \
    | grep -v "^$" | tail -1 | tr -d ' ')
chk "VERIFY(1件目): COPY_STATUS=VERIFIED" "VERIFIED" "${COPY1_STS}"

# 1件目検証時点では ARCHIVE_LOG はまだ VERIFIED でない（2件目が未検証）
INTERIM_COL_STS=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
SELECT COLLECT_STATUS FROM ARCHIVE_LOG
WHERE ARCHIVE_LOG_ID = ${ARCHIVE_LOG_ID:-0};" \
    | grep -v "^$" | tail -1 | tr -d ' ')
chk "1件目VERIFIED後: ARCHIVE_LOG.COLLECT_STATUS=RECEIVED（未変更）" "RECEIVED" "${INTERIM_COL_STS}"

# 2件目（8TB_SSD）を VERIFIED へ → 全コピーが VERIFIED になり ARCHIVE_LOG も更新
mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON SERVEROUTPUT ON
BEGIN
  PKG_MIG_ADMIN.VERIFY_ARCHIVE_LOG_COPY(
    p_copy_id  => ${ALC_ID2:-0},
    p_checksum => 'sha256:def456ghi789'
  );
END;
/" > /dev/null
...
WHERE ARCHIVE_LOG_COPY_ID = ${ALC_ID2:-0};" \
    | grep -v "^$" | tail -1 | tr -d ' ')
chk "VERIFY(2件目): COPY_STATUS=VERIFIED" "VERIFIED" "${COPY2_STS}"

# 全コピー VERIFIED → ARCHIVE_LOG.COLLECT_STATUS = VERIFIED になること
COLLECT_STS=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
SELECT COLLECT_STATUS FROM ARCHIVE_LOG
WHERE ARCHIVE_LOG_ID = ${ARCHIVE_LOG_ID:-0};" \
    | grep -v "^$" | tail -1 | tr -d ' ')
chk "全コピーVERIFIED後: ARCHIVE_LOG.COLLECT_STATUS=VERIFIED" "VERIFIED" "${COLLECT_STS}"
...
if [ -n "${VER_HIST}" ] && [ "${VER_HIST}" -gt 0 ] 2>/dev/null; then
    echo "  [OK] VERIFY_ARCHIVE_LOG_COPY: MIG_STATUS_HISTORY 記録あり(${VER_HIST}件)"
else
...
# ARCHIVE_LOGが0件のRUNへの呼び出しは ORA-20003
NO_LOG_ERR=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON SERVEROUTPUT ON
BEGIN
...
# ===========================================================================
# T13: PKG_MIG_ADMIN.FAIL_DATAPUMP_JOB テスト（MED#5）
# ===========================================================================
...
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
INSERT INTO DATAPUMP_JOB (MIG_RUN_ID, JOB_NAME, OPERATION)
VALUES (${PKG_RUN_ID:-0}, 'PKG_TEST_FAIL_JOB', 'IMPORT');
COMMIT;" > /dev/null

FAIL_JOB_ID=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
SELECT DATAPUMP_JOB_ID FROM DATAPUMP_JOB
WHERE JOB_NAME = 'PKG_TEST_FAIL_JOB';" | num)

# PLANNED → RUNNING
mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON SERVEROUTPUT ON
BEGIN
  PKG_MIG_ADMIN.START_DATAPUMP_JOB(p_job_id => ${FAIL_JOB_ID:-0});
END;
/" > /dev/null
...
BEGIN
  PKG_MIG_ADMIN.FAIL_DATAPUMP_JOB(
    p_job_id        => ${FAIL_JOB_ID:-0},
    p_error_message => 'FAIL-TEST-ERR-001'
  );
END;
/" > /dev/null
...
FAIL_STS=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
SELECT STATUS FROM DATAPUMP_JOB WHERE DATAPUMP_JOB_ID = ${FAIL_JOB_ID:-0};" \
    | grep -v "^$" | tail -1 | tr -d ' ')
chk "FAIL_DATAPUMP_JOB: STATUS=FAILED" "FAILED" "${FAIL_STS}"

# REMARKS に記録があること（IS NOT NULL）
FAIL_REMARKS_CNT=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
SELECT COUNT(*) FROM DATAPUMP_JOB
WHERE DATAPUMP_JOB_ID = ${FAIL_JOB_ID:-0} AND REMARKS IS NOT NULL;" | num)
chk "FAIL_DATAPUMP_JOB: REMARKSが記録されていること" "1" "${FAIL_REMARKS_CNT}"

FAIL_HIST_CNT=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
SELECT COUNT(*) FROM MIG_STATUS_HISTORY
WHERE TABLE_NAME = 'DATAPUMP_JOB' AND NEW_STATUS = 'FAILED';" | num)
if [ -n "${FAIL_HIST_CNT}" ] && [ "${FAIL_HIST_CNT}" -gt 0 ] 2>/dev/null; then
    echo "  [OK] FAIL_DATAPUMP_JOB: MIG_STATUS_HISTORY 記録あり(${FAIL_HIST_CNT}件)"
else
    echo "  [NG] FAIL_DATAPUMP_JOB: MIG_STATUS_HISTORY 記録なし"
    PASS=0
fi

# COMPLETED 状態のジョブに FAIL_DATAPUMP_JOB を呼ぶと ORA-20002
COMP_FAIL_ERR=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON SERVEROUTPUT ON
BEGIN
  PKG_MIG_ADMIN.FAIL_DATAPUMP_JOB(
    p_job_id        => ${PKG_DP_JOB_ID:-0},
    p_error_message => 'should-fail'
  );
END;
/" 2>&1)
if echo "${COMP_FAIL_ERR}" | grep -q "ORA-20002"; then
    echo "  [OK] FAIL_DATAPUMP_JOB(COMPLETED): 事前条件違反 -> ORA-20002"
else
    echo "  [NG] FAIL_DATAPUMP_JOB(COMPLETED): ORA-20002 が発生しなかった"
    echo "       出力: ${COMP_FAIL_ERR}"
    PASS=0
fi
...
# ===========================================================================
# T14: PKG_MIG_ADMIN.SET_TARGET_END_SCN テスト（MED#5）
# ===========================================================================
...
TARGET_SCN=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
SELECT TARGET_END_SCN FROM MIGRATION_RUN WHERE MIG_RUN_ID = ${PKG_RUN_ID:-0};" | num)
chk "SET_TARGET_END_SCN: TARGET_END_SCN=9999999" "9999999" "${TARGET_SCN}"

OVERWRITE_SCN_ERR=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON SERVEROUTPUT ON
BEGIN
  PKG_MIG_ADMIN.SET_TARGET_END_SCN(
    p_run_id         => ${PKG_RUN_ID:-0},
    p_target_end_scn => 8888888
  );
END;
/" 2>&1)
if echo "${OVERWRITE_SCN_ERR}" | grep -q "ORA-20001"; then
    echo "  [OK] SET_TARGET_END_SCN 2回目: 上書き禁止 -> ORA-20001"
else
    echo "  [NG] SET_TARGET_END_SCN 2回目: ORA-20001 が発生しなかった"
    echo "       出力: ${OVERWRITE_SCN_ERR}"
    PASS=0
fi
...
# ===========================================================================
# T15: PKG_MIG_ADMIN.UPDATE_LAST_APPLIED_SCN テスト（MED#5）
# ===========================================================================
...
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON SERVEROUTPUT ON
BEGIN
  PKG_MIG_ADMIN.UPDATE_LAST_APPLIED_SCN(
    p_run_id => ${PKG_RUN_ID:-0},
    p_scn    => 5000000
  );
END;
...
APPLIED_SCN=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
SELECT LAST_APPLIED_SCN FROM MIGRATION_RUN WHERE MIG_RUN_ID = ${PKG_RUN_ID:-0};" | num)
chk "UPDATE_LAST_APPLIED_SCN: LAST_APPLIED_SCN=5000000" "5000000" "${APPLIED_SCN}"

# 後退（4000000）→ ORA-20004
REGRESS_ERR=$(mctl_sql "
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON SERVEROUTPUT ON
BEGIN
  PKG_MIG_ADMIN.UPDATE_LAST_APPLIED_SCN(
    p_run_id => ${PKG_RUN_ID:-0},
...
END;
/" 2>&1)
if echo "${REGRESS_ERR}" | grep -q "ORA-20004"; then
...
SET PAGESIZE 0 FEEDBACK OFF HEADING OFF TRIMSPOOL ON
SELECT LAST_APPLIED_SCN FROM MIGRATION_RUN WHERE MIG_RUN_ID = ${PKG_RUN_ID:-0};" | num)
chk "UPDATE_LAST_APPLIED_SCN(前進): LAST_APPLIED_SCN=5000001" "5000001" "${FWD_SCN}"
